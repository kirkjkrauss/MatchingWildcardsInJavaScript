// Compares two text strings, the first of which may contain wildcards 
// ('*' or '?').
function FastWildCompare(w, t){
	let i=0;let j;let k;let l;
	do{if(t[i]==undefined){if(w[i]!=undefined)return false;else	return true;}else if(w[i]=='*'){j=i;while(w[++i]=='*')continue;if(w[i]==undefined)return true;if(w[i]!='?'){while(w[i]!=t[j]){if(t[++j]==undefined)return false;}}k=i;l=j;break;}else if(w[i]!=t[i] && w[i]!='?')return false;++i;}while(true);
